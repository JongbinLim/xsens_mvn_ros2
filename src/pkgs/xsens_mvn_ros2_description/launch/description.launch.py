...
def generate_launch_description():
    ...
    urdf_publisher_node = LifecycleNode(
        package='xsens_mvn_ros2_description',
        executable='xsens_mvn_ros2_urdf_publisher_node',
        name='xsens_urdf_publisher',
        namespace=LaunchConfiguration('namespace'),
        output='screen',
        parameters=[
            PathJoinSubstitution([
                FindPackageShare('xsens_mvn_ros2_description'),
                'config', 'xsens_urdf_publisher_node.yaml']),
    ...
    activate_handler = RegisterEventHandler(
        ...
        event_handler=OnStateTransition(
            target_lifecycle_node=urdf_publisher_node,
            goal_state='inactive',
            handle_once=True,
            entities=[
                EmitEvent(
